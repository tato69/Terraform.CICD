...
resource "azurerm_public_ip" "test2" {
  name                         = "CI-CD-pipeline-test-pip02"
  location                     = "West US 2"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  public_ip_address_allocation = "static"
...
resource "azurerm_network_interface" "test2" {
  name                = "CI-CD-pipeline-test-nic02"
  location            = "West US 2"
  resource_group_name = "${azurerm_resource_group.test.name}"

  ip_configuration {
    name                          = "CI-CD-pipeline-test-conf02"
    subnet_id                     = "${azurerm_subnet.test.id}"
    private_ip_address_allocation = "static"
...
storage_os_disk {
    name              = "CI-CD-pipeline-test-disk02"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Premium_LRS"
}
...
os_profile {
    computer_name  = "CI-CD-pipeline-test-vm02"
    admin_username = "ariso001a"
	admin_password = "Password123"
}
...
provisioner "file" {
  source      = "./pp_agent_apache.bash"
  destination = "/tmp/pp_agent_apache.bash"
}

provisioner "remote-exec" {
  inline = [
    "chmod +x /tmp/pp_agent_apache.bash",
    "/tmp/pp_agent_apache.bash",
  ]
}
